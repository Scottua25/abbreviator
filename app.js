...
export function buildShortDescription(longText, abbreviations, maxLen = 30) {
  maxLen = 30;
...
export function rankCandidates(description, records, fields = ['name', 'description'], limit = 5) {
...
export function browseFieldOptions(data, fieldName, searchText = '', limit = 1000) {
...
export function suggestFields(description, data, limit = 5) {
...
export async function loadData(url = './data/eco-reference-data.json') {
  try {
...
export function displayValueForField(fieldName, row) {
  if (!row) return '';
